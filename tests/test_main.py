...
from deep_knowledge_agent.main import handler
...
async def test_handler_returns_response():
    ...
    with (
        patch("deep_knowledge_agent.main._initialized", True),
        patch("deep_knowledge_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response),
    ):
...
async def test_handler_with_multiple_messages():
    ...
    with (
        patch("deep_knowledge_agent.main._initialized", True),
        patch("deep_knowledge_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run,
    ):
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with (
        patch("deep_knowledge_agent.main._initialized", False),
        patch("deep_knowledge_agent.main.initialize_agent", new_callable=AsyncMock) as mock_init,
        patch("deep_knowledge_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run,
        patch("deep_knowledge_agent.main._init_lock", new_callable=MagicMock()) as mock_lock,
    ):
...
async def test_handler_with_research_query():
    ...
    messages = [
        {
            "role": "user",
            "content": "Research the impact of quantum computing on cryptography",
        }
    ]

    mock_response = MagicMock()
    mock_response.run_id = "research-run-id"
    mock_response.content = "Research analysis generated successfully."
    ...
    assert result.content == "Research analysis generated successfully."
